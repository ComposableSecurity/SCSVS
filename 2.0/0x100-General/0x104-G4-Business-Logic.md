...
| **G4.15** | Verify that signature-authorized actions (permits, meta-transactions, gasless approvals) include the chain ID, a nonce, and an expiry, and cannot be replayed across forks or after expiry. |
| **G4.16** | Verify that protocol invariants hold when the function is called atomically inside a flash loan, flash mint, or callback. |
| **G4.17** | Verify that token-receiver callbacks (ERC-777 `tokensReceived`, ERC-721/1155 `onReceived`, ERC-3156 flash callbacks, Uniswap V3/V4 swap & flash callbacks) cannot break the calling contract's invariants through read-only or cross-function reentrancy. |
| **G4.18** | Verify that operations that depend on `block.timestamp` are robust to -/+15 s proposer manipulation and to L2 sequencers that may publish identical timestamps for consecutive blocks. |
