...
| **G10.5** | Verify that the 63/64 gas-forwarding rule (EIP-150) is taken into account when a child call must have enough gas to complete; otherwise a caller can grief by giving just enough gas. |
| **G10.6** | Verify that calldata-heavy operations on L2s account for L1 data-availability costs (EIP-4844 blob costs) and do not encourage griefing through oversized calldata. |
| **G10.7** | Verify that critical paths do not depend on opcode gas costs that have changed across hardforks (EIP-2929 cold/warm access, EIP-3529 refund reduction). |
